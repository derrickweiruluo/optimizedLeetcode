...
每个node所在的subtree都是一个部分解，每个node return 当前node + max(左右子树)
'''
class Solution: #1:
    ...
    def dfs(self, node):
        if not node: 
            return 0
        left = max(0, self.dfs(node.left))  #防止走一个sum是负的subtree
        right = max(0, self.dfs(node.right))#防止走一个sum是负的subtree
