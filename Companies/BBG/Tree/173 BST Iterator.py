...
class BSTIterator:
    ...
    def next(self) -> int:
        ...
        while self.cur:
            ...
            # case 2: 有左子树, 走左子树的最右边 :: cur.left.right.right
            # BST inorder 顺序
            else:
                ...
                if not predecessor.right:  # 第一次访问，先建立parent pointer到cur
                    predecessor.right = self.cur
                    self.cur = self.cur.left

                else: # 第二次访问，要删掉parent reference
                    res = self.cur.val
                    predecessor.right = None
                    self.cur = self.cur.right  # 搜索完左子树，搜右子树
                    break
...
class BSTIterator:  #O(1) time average of N next call, O(H) space
